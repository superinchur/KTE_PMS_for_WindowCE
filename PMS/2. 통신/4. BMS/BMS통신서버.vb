...
Public Class BMS통신서버
    ...
    Public Overloads Sub ServiceStart()
        ...
        ' 서버를 시작시킨다.
        Try
            Dim ipep As IPEndPoint = New IPEndPoint(IPAddress.Any, BMS_수신포트)
        ...
        Try
            While True
                ...
                If client.Connected Then ' 소켓이 연결되어 있다면 
                    Dim pDevice As BMS통신차일드 = New BMS통신차일드(client)
        ...
        If Not alClientList Is Nothing Then
            For nCnt As Integer = 0 To alClientList.Count - 1
                Try
                    ...
                    Dim pObj As BMS통신차일드 = alClientList.Item(nCnt)
                    pObj.ServiceStop()
                    pObj = Nothing
